...
def openChanWithNode(node):
    ...
    try:
        ...
        print(
            termPrint("OPENED with TX:\t", bcolors.OKGREEN) + termPrint(str(response.funding_txid_bytes.encode('hex')),
                                                                        bcolors.WARNING))
...
def connectAllNodes(networkData_nodes, stub, macaroon):
    ...
    for node in nodesToTry:
        ...
        print("[NetView Upkeep][" + str(index) + "/" + str(len(nodesToTry)) + "] Target: " + termPrint(
...
def connect_all_peers(graph_filePath, stub, macaroon):
    ...
    currentPendingParties = [pending_chan.channel.remote_node_pub for pending_chan in
    ...
    newNodes = [node for node in peerList if
                not node.pub_key in currentCounterParties and not node.pub_key in currentPendingParties]

    print(termPrint("Channels now:\t", bcolors.OKGREEN) + str(len(thunder.getCurrentChannels(stub, macaroon))))
    print(termPrint("Channels soon:\t", bcolors.OKGREEN) + str(
        len(thunder.getPendingChannels(stub, macaroon).pending_open_channels)))
    print(termPrint("Connected to:\t", bcolors.OKGREEN) + str(len(peerList)) + termPrint("\tNew Peers:\t",
                                                                                         bcolors.OKGREEN) + str(
        len(newNodes)))

...
from _scripts.utils_db_upkeep import *
...
def generate_chart_dataset(description_string, target, logged_time):
    print(f"Generating dataset for:\t{description_string}\t for time\t{logged_time.strftime('%Y-%m-%d %H:%M:%S')}")

    dataset_template = {"label": description_string, "data": [],
                        "backgroundColor": 'rgba(255, 159, 64, 0.2)',
                        "borderColor": 'rgba(255, 159, 64, 1)'}
    results = []

    if description_string == "node_capacity":
        ...
        new_dataset["data"] = [{"x": x.date_logged.strftime("%Y-%m-%d %H:%M:%S:%f"), "y": x.capacity} for x in
                               data_query_result]
        # new_dataset["data"] = [ {"x": time, "y":data_point } for time,data_point in zip(str_dates,avg_chan_size)]
        results.append(new_dataset)
    elif description_string == "node_channels":
        new_dataset = dataset_template
        options = "options: { scales: { xAxes: [{ type: 'time' }] }}"
        labels = ""

        data_query_result = Node.objects.filter(pub_key=target).order_by("date_logged")
        new_dataset["data"] = [{"x": x.date_logged.strftime("%Y-%m-%d %H:%M:%S"), "y": x.channels} for x in
                               data_query_result]
        # new_dataset["data"] = [ {"x": time, "y":data_point } for time,data_point in zip(str_dates,avg_chan_size)]
        results.append(new_dataset)
...
def get_graph_cache(network="testnet"):
    file_name = "cachedLocations-" + network + ".json"
    return json.loads(open(file_name).read())


def get_network_stats(target_config_file):  # The output from the output of "lncli getnetworkinfo"
    enc = 'utf-8'
...
def index(request):
    ...
    try:  # First set of statistics LOCAL NODE NETWORK
        networkStats, date_logged = get_network_stats(
            site_config["network_info_path"])  # Date_logged is unix timestamp as float
        # Convert Sat to BTC
    ...
    try:
        networkStats, date_logged = get_network_stats(
            site_config["network_info_" + othernet])  # Date_logged is unix timestamp as float
        # Convert Sat to BTC
        networkStats["total_network_capacity"] = str(float(networkStats["total_network_capacity"]) / 10 ** 8) + " BTC"
        networkStats["max_channel_size"] = str(float(networkStats["max_channel_size"]) / 10 ** 8) + " BTC"
        networkStats["avg_channel_size"] = str(networkStats["avg_channel_size"] / 10 ** 8) + " BTC"
    ...
    return render(request, 'nodes/index.html', {"networkStats_testnet": list(netstats["testnet"][0].items()),
                                                "networkStats_mainnet": list(netstats["mainnet"][0].items()),
                                                "last_update_testnet": netstats["testnet"][1],
                                                "last_update_mainnet": netstats["mainnet"][1]})
...
def visualiser(request, network, date_logged=""):
    ...
    print("Got cache and node and edge data " + str(len(networkData["nodes"])) + " with edges " + str(
        len(networkData["edges"])))
    ...
    return render(request, 'nodes/visualiser.html',
                  {"jsonData": json.dumps({"nodes": n, "edges": e}, default=str), "cachedPoz": json.dumps(nodePoz),
                   "network": network, "date_logged": date_logged, "chan_count": len(e), "nodes_count": len(n)})
...
def prepareGraphData(nodeEntries, edgeEntries):
    edgeList = [{"chan_id": edge.chan_id, "node1_pub": edge.node1_pub.pub_key, "node2_pub": edge.node2_pub.pub_key,
                 "capacity": edge.capacity} for edge in edgeEntries]

    nodeList = [{"alias": nodeEntry.alias, "pub_key": nodeEntry.pub_key, "color": nodeEntry.color,
                 "channels": nodeEntry.channels} for nodeEntry in nodeEntries]
    return nodeList, edgeList
...
def nodes(request, network, date_logged=""):
    ...
    return render(request, 'nodes/nodes.html', {"nodes": nodes_page, "network": network, "date_logged": date_logged,
                                                "date_logged_unix": date_logged_unix})
...
def channels(request, network, date_logged=""):
    ...
    return render(request, 'nodes/channels.html',
                  {"channels": channels_page, "network": network, "date_logged": date_logged,
                   "date_logged_unix": date_logged_unix})
...
def nodes_detail(request, nodePubKey, network, date_logged=""):
    ...
    js_dataset_capacity, js_options, js_labels_capacity = generate_chart_dataset("node_capacity", nodePubKey,
                                                                                 date_logged)
    js_dataset_chans, js_options, js_labels_chans = generate_chart_dataset("node_channels", nodePubKey, date_logged)

    data_dates = [
        {"date_display": x["date_logged"].strftime("%Y-%m-%d %H:%M:%S"), "date_unix": x["date_logged"].strftime("%s")}
        for x in Node.objects.filter(pub_key=nodePubKey).values("date_logged").distinct().order_by('date_logged')]
    ...
    return render(request, 'nodes/nodes_detail.html',
                  {"nodes": json.dumps(prepedNodes),
                   ...
                   "date_logged": {"date_display": date_logged.strftime("%Y-%m-%d %H:%M"),
                                   "date_unix": date_logged.strftime("%s")},
                   "network": network})
...
def channel_detail(request, chanID, date_logged="", network="testnet"):
    ...
    data_dates = [{"date_display": x["date_logged"].strftime("%Y-%m-%d"), "date_unix": x["date_logged"].strftime("%s")}
                  for x in
                  Channel.objects.filter(chan_id=chanID).values("date_logged").distinct().order_by('date_logged')]
    ...
    return render(request, 'nodes/channels_detail.html', {"nodes": json.dumps(n),
                                                          ...
                                                          "data_dates": data_dates,
                                                          "date_logged": {
                                                              "date_display": date_logged.strftime("%Y-%m-%d %H:%M"),
                                                              "date_unix": date_logged.strftime("%s")},
                                                          "network": network})
...
def getEdgeConnections(networkGraph, edgeID):
    ...
    otherChannelsForNodes = list(
        networkGraph["edges"].filter(node1_pub=edgeEntry.node1_pub, node2_pub=edgeEntry.node2_pub).exclude(
            chan_id=edgeID)) + list(
        networkGraph["edges"].filter(node1_pub=edgeEntry.node2_pub, node2_pub=edgeEntry.node1_pub).exclude(
            chan_id=edgeID))
...
def getNodeEdges(networkGraph, nodeEntry):
    vecinNodesDuplicates = [nodeEntry] + [x.node2_pub for x in nodeEntry.node1_pub.all()] + [x.node1_pub for x in
                                                                                             nodeEntry.node2_pub.all()]
...
def search(request, network):
    ...
    if request.method == 'GET':
        ...
        try:
            raw_search_term = request.GET[
                'search_term'].strip()  # Note that django does automatic html characted sanitising
            if (raw_search_term == ""):
            ...
            search_result = "Found " + str(len(possibleMatchesNodes)) + " matching nodes and " + str(
                len(possibleMatchesEdges)) + " channels"
            search_term = "Showing results for search term: \"" + raw_search_term + "\" on the Bitcoin " + network + "."
        except Exception as e:
            print(e)
            search_term = "No search term found"
            search_result = "Please use the navbar to find nodes and edges. " \
                            "You can use their public keys, channel identifiers or aliases to search. " \
                            "Alternatively try, the other network."

        pmNode, pmEdges = prepareGraphData(possibleMatchesNodes, possibleMatchesEdges)

        return render(request, 'nodes/search.html',
                      {"searchInfo": search_term, "foundNodes": pmNode, "foundChannels": pmEdges,
                       "strResults": search_result, "network": on_network})
    else:
        index(request)

    #         status = Nodes.objects.filter(node__icontains=node)
